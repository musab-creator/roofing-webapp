        ...
        <TabsPrimitive.Root
          ...
          <TabsPrimitive.List className="flex items-center justify-around">
            {navItems.map((item) => {
              ...
              return (
                <TabsPrimitive.Trigger
                  key={item.id}
                  value={item.id}
                  className="flex-1 flex flex-col items-center justify-center py-1 rounded-[1.5rem] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring motion-safe:transition-all motion-safe:duration-200">
                  <div
                    className={`flex flex-col items-center justify-center gap-1 px-3 py-1 rounded-2xl motion-safe:transition-all motion-safe:duration-200 ${isActive ? 'bg-gray-100 dark:bg-slate-800 shadow-sm' : 'bg-transparent'}`}>
                    {/* Icon + badge wrapper */}
                    ...
                    {/* Label */}
                    <span
                      className={`text-[10px] leading-none motion-safe:transition-all motion-safe:duration-200 ${isActive ? 'text-primary font-semibold' : 'text-muted-foreground font-medium'}`}>
                      {item.label}
                    </span>
                  </div>
                </TabsPrimitive.Trigger>
            ...
            <button
              ...
              <div className="flex items-center justify-center rounded-full px-3 py-1 bg-transparent">
                <SearchIcon
                  size={18}
                  strokeWidth={1.8}
                  className="text-muted-foreground motion-safe:transition-all motion-safe:duration-200"
                />
              </div>
              <span className="text-[10px] leading-none text-muted-foreground font-medium motion-safe:transition-all motion-safe:duration-200">
                Search
              </span>
            </button>
          </TabsPrimitive.List>
